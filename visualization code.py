...
fig.update_layout(
    ...
    title=dict(
        text="Most bicycle accidents occur during the main active hours of the day",
        x=0.5,
        xanchor="center"
    ),
...
##save as HTML
save_path = asksaveasfilename(
    title="Save bar chart as HTML",
    defaultextension=".html",
    initialfile="bar_time_severity.html",
...
if save_path:
    ...
    print(f"✔ Saved:\n{save_path}")
    webbrowser.open(Path(save_path).as_uri())
else:
    print("Save cancelled.")
...
fig.update_layout(
    template="plotly_white",
    title=dict(
        text="Accident frequency on weekdays is especially high during active hours",
        x=0.5,
        xanchor="center"
    ),
...
fig.update_layout(
    template="plotly_white",
    title=dict(text="Fatal bicycle accidents tend to occur at higher speed limits", x=0.5, xanchor="center"),
    xaxis=dict(
...
##save HTML
save_path = asksaveasfilename(
    title="Save box plot as HTML",
    defaultextension=".html",
    initialfile="box_speedlimit_severity_black_orange_median.html",
    filetypes=[("HTML file", "*.html")]
)
...
def make_title(sev: str) -> str:
    return f"During nighttime hours, the share of serious and fatal injuries in bicycle accidents increases."
